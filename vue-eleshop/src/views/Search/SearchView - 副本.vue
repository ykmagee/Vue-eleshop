...
<script>
	...
	export default {
	  ...
	  mounted(){
	  	if (this.userInfo) {
	  		console.log(this.userInfo,this.userInfo.searchHistory)
	  	}
	  	
	  	// this.$store.dispatch('getUserInfo');
	  	// 读取本地搜索历史记录，若存在就更新到searchHistory中
	  	const localSearch=Store.get('searchHistory');
	  	this.$nextTick(()=>{
	  		// 若用户已登录则获取用户信息中的搜索历史
	  		if (this.userInfo.searchHistory) {
	  			this.searchHistory=this.userInfo.searchHistory;
	  		}
	  		// 若用户没有登录则获取本地的搜索历史
	  		if (!this.userInfo._id && localSearch) {
		  		this.searchHistory=localSearch;	  		
		  	}
		  	console.log(Boolean(this.userInfo.searchHistory),Boolean(!this.userInfo._id && localSearch))
		  	// console.log(this.userInfo,this.userInfo.searchHistory)
	  	})
	  	// if (localSearch) {
	  	// 	this.searchHistory=localSearch;	  		
	    ...
	    search (tagName) {
	      ...
	      if (keyword) {
	        ...
	        if (findIndex === -1) {
	        	...
	        	// 若用户已登录则将搜索历史记录在数据库中更新
	        	if (this.userInfo._id) {
	        		reqUpdateHistory(this.userInfo._id, this.searchHistory);
	        		console.log(this.searchHistory,this.userInfo.searchHistory)
	        	}else{
	        		// 否则将搜索历史数组保存到本地localStorage
